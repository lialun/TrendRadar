...
class NewsAnalyzer:
        ...
        if self.ctx.config["STORAGE"]["FORMATS"]["HTML"]:
            ...
            html_file = self.ctx.generate_html(
                ...
                report_metadata={
                    ...
                    "rss_total_count": getattr(self, "_rss_total_count", 0),
                    "rss_source_total": getattr(self, "_rss_source_total", 0),
                    "rss_source_failed": getattr(self, "_rss_source_failed", 0),
                },
    ...
    def _send_notification_if_needed(
        ...
        if (
            ...
            # 注入元数据（用于推送头部展示）
            report_data["hotlist_total"] = getattr(self, "_hotlist_total_count", news_count)
            report_data["platform_total"] = len(self.ctx.platform_ids)
            report_data["rss_matched_count"] = getattr(self, "_rss_matched_count", rss_count)
            report_data["rss_total_count"] = getattr(self, "_rss_total_count", 0)
            report_data["rss_source_total"] = getattr(self, "_rss_source_total", 0)
            report_data["rss_source_failed"] = getattr(self, "_rss_source_failed", 0)
